    ...
    def radix_sort(a):
        ...
        if len(a)>1:
            ...
            for i in range(1,maximum+1):
                Sorts._radix_sort_aux(a, i)
        return 

    @staticmethod       
    def _radix_sort_aux(a,i): 
        ...
        if len(a) > 0:
            for x in range(0,len(a),1):
                ...
                while i < len(a) - 1:
                    left = i - 1
                    right = i + 1
                    if left >= 0 and a[left]<= a[i] and a[i] <= a[right]:
                        i+=1
                    else:
                        if left>= 0 and a[left]>a[i]:
                            Sorts._swap(a, left, i)
                        if a[i]>a[right]:
                            Sorts._swap(a, i, right)
                        if i-1 < 0:
                            i+=1
                        else:
                            i-=1
