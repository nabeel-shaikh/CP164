...
class List:
    ...
    def insert(self, i, value):
        ...
        if self._is_valid_index(i):
            if i == 0:
                next = self._front
                self._front = _List_Node(value,next)
            else:
                if i<0:
                    i = self._count + i
                prev = None
    ...
    def max(self):
        ...
        max = deepcopy(self._front._value)
        curr = self._front
        while curr is not None:
            if curr._value > max:
                max = deepcopy(curr._value)
            curr = curr._next
        return max
    ...
    def min(self):
        ...
        """
        assert self._front is not None, "Cannot find maximum of an empty list"
        min = deepcopy(self._front._value)
        curr = self._front
        while curr is not None:
            if curr._value < min:
                min = deepcopy(curr._value)
            curr = curr._next
        return min
